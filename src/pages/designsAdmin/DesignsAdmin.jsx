...
const DesignsAdmin = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(apiRoute + "/read/designs",setRequestConfig()).then(re=>re.json()).then(data=>{
      setDesigns(data)
    }).catch(err=>{
      console.log(err);
    }
  ...
  async function editorModalSubmited(e){
    ...
    if (dataInEditor.id) {
      ...
      if (dataObj.img.size > 0) {
        ...
        setLoading(true);
        await fetch(`${apiRoute}/update_design/${design.id}/img`,setRequestConfig())
        .then(response => response.json())
        .then((data) => {console.log(data);})  
        .catch(error => console.log(error))
        .finally(()=>{
        ...
        <div className="row gap-2">
          {
            ...
            onDelete={design => {console.log(design)}}
